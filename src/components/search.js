...
AFRAME.registerComponent('search', {
  ...
  search: function (query) {
    ...
    let url = `/api/search/text/CURRENT_PAGE_INDEX?sortOrder=Rating&automapper=true&q=${encodeURIComponent(query)}`;

    if (this.data.playlist) {
      url = `/api/playlists/id/${this.data.playlist}/CURRENT_PAGE_INDEX`;
    } else if (this.data.genre) {
      const genreMap = {
        'Pop': 'pop',
        'R&B': 'rb',
      ...
      url = `/api/search/text/CURRENT_PAGE_INDEX?sortOrder=Rating&automapper=true&tags=${encodeURIComponent(tag)}`;
    } else {
